...
- **Auth check first.** Multiple `gh` accounts. Run `gh auth status`. Switch
  with `gh auth switch --user <login>` if wrong. Confirm correct account before
  any write.
...
1. **Verify auth.** `gh auth status`. Switch if wrong.

2. **Confirm base branch.** Default is repo default (usually `main`/`master`).
   User specified other base: use it.

3. **Read branch diff vs base.** `git diff <base>...HEAD` and `git log
   <base>..HEAD`. Understand full branch, not just last commit.

4. **Compose title + body** per format above.

5. **Push branch if needed.** Check `git status` for unpushed commits. Push to
   origin with upstream if first push.

6. **Show, then create** (visibility gate):
...
7. **Pass the body via heredoc on stdin** to preserve formatting:
...
- **Auth check first**: wrong `gh` account on a write op is hard to undo (PR
  opened under personal account on work repo, etc).
